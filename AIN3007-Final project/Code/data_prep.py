...
import cv2
#from config import WSI_IMAGES_DIR, XML_ANNOTATIONS_DIR, ROI_PATCHES_DIR
...
from.
'''
WSI_IMAGES_DIR = '/Volumes/moedisk/AIN3007_project/train_valid_data/Breast3__ihc'
XML_ANNOTATIONS_DIR = '/Volumes/moedisk/AIN3007_project/train_valid_data/Breast3__ihc'
ROI_PATCHES_DIR = '/Users/hamodyx0/Documents/school/deeplearning/DLproject/extracted_patches/forground'
PATCH_SIZE = 256
...
def extract_patches_from_wsi(slide, rois, output_dir, patch_size):

    lowest_level = 3 #slide.level_count - 1  # Lowest resolution level
    ...
    for i, roi in enumerate(rois):
        ...
        for row in range(y, y + h, patch_size):
            for col in range(x, x + w, patch_size):
                patch = slide.read_region((int(col * downsample), int(row * downsample)), lowest_level, (patch_size, patch_size))
                patch = np.array(patch)[:, :, 0:3]  # Drop the alpha channel
                cv2.imwrite(os.path.join(output_dir, f'roi_{i}_{row}_{col}.png'), patch)
